...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**API key**")
    st.caption("Set `OPENAI_API_KEY` as an environment variable or Streamlit secret.")
    show_key = st.toggle("Show key field (optional)", value=False)
    api_key_override = ""
    if show_key:
        api_key_override = st.text_input("OPENAI_API_KEY", type="password", value="")
...
with tab_ai:
    ...
    if st.button("Generate attributes", type="primary", disabled=not product.strip()):
        key = api_key_override.strip() or _get_api_key()
        if not key:
...
if run:
    ...
    interpret = st.button("Generate interpretation")
    if interpret:
        with st.spinner("Writing interpretation…"):
            try:
                narrative = interpret_results_with_ai(
                    api_key=key,
                    model_name=str(model_name).strip() or "gpt-4.1-mini",
